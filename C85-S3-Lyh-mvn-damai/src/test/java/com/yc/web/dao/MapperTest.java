...
public class MapperTest {
	...
	public void test1() throws IOException {
		...
		List<DmProduct> list = session.selectList("com.yc.web.dao.DmProductMapper.selectAll");
		/**
		 * 	使用断言进行结果的判断
		 *  true 表示的期望值
		 *  list.size()>0 是实际值
	...
	public void test2() throws IOException {
		...
		dc.setCname("测试分类");
		dc.setPid(1);
		// session.insert("com.yc.web.dao.DmProductMapper.insert",dc);
	...
	public void test3() throws IOException {
		...
		dc.setPid(1);
		//session.update("com.yc.web.dao.DmProductMapper.update",dc);
		DmCategoryMapper mapper = session.getMapper(DmCategoryMapper.class);
		mapper.update(dc);
		// 不commit, 会话会在关闭自动回滚
		session.commit();
	...
	public void test4() throws IOException {
		...
		dc.setCname("修改后的测试分类");
		dc.setPid(1);*/
		//session.delete("com.yc.web.dao.DmProductMapper.delete",dc);
		DmCategoryMapper mapper = session.getMapper(DmCategoryMapper.class);
		mapper.delete(44);
		// 不commit, 会话会在关闭自动回滚
		session.commit();
	...
	public void test11() throws IOException {
		...
		int[] cids = {1000};
		/**
		 * Cache Hit Ratio [com.yc.web.dao.DmProductMapper]: 0.0
		 * 	缓存命中:  当前查询结果在缓存出现的概率
		 */
